...
final class BencodeParserXCTests: XCTestCase {
  ...
  func testParseDictionary() throws {
    // d3:foo3:bar  → key "foo" = "bar"
    //  3:bazi42ee  → key "baz" = 42
    let data = Data("d3:foo3:bar3:bazi42ee".utf8)
    let result = try BencodeParser.decode(data)

    let dict = try XCTUnwrap(result.dictionaryValue, "Expected a dictionary at the top level")
    XCTAssertEqual(dict["foo"]?.stringValue, "bar")
    XCTAssertEqual(dict["baz"]?.integerValue, 42)
    XCTAssertNil(dict["missing"], "Absent key should return nil")
  }
  ...
  func testRoundTripEncodingAndDecoding() throws {
    ...
    XCTAssertEqual(decodedDict["creation date"]?.integerValue, 1_234_567_890)
