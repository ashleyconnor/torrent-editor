...
final class TorrentFile {
    ...
    var pieceLength: Int = 262_144 // 256 KB default
        ...
        if let files = info["files"]?.listValue {
            ...
            for fileValue in files {
                guard let fileDict = fileValue.dictionaryValue,
                      ...
                      let pathList = fileDict["path"]?.listValue else {
                    continue
                }
    ...
    func addFile(url: URL) throws {
        ...
        guard let fileSize = attributes[.size] as? Int else {
            throw NSError(domain: "TorrentFile", code: 1, userInfo: [NSLocalizedDescriptionKey: "Could not determine file size"])
        }
